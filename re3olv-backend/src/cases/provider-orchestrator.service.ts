...
export class ProviderOrchestratorService {
  ...
  async getActiveProvider(countryCode: string, category: string, organizationId: string) {
    ...
    if (!provider) {
      this.logger.warn(`No active provider found for ${countryCode} [${category}]. Implementing default fallback.`);
      ...
      if (countryCode === 'MA' && category === 'BUSINESS') {
        return {
          ...
          triggerSurvey: true,
          message: 'Official Register unavailable in Morocco region. Triggering Cash-Flow Proxy survey.'
        };
      }
  ...
  async toggleProvider(organizationId: string, providerId: string, active: boolean) {
    if (active) {
