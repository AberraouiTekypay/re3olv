...
export default async function ConfirmationPage({ params }: { params: { caseId: string } }) {
  ...
  try {
    ...
    return (
      <div className="container mx-auto py-24 px-4 flex justify-center">
        <Card className="max-w-md w-full border-t-8 border-t-indigo-500 shadow-xl">
          <CardHeader className="text-center pb-2">
            <div className="flex justify-center mb-4">
              <Sparkles className="w-16 h-16 text-indigo-500 animate-pulse" />
            </div>
            <CardTitle className="text-3xl font-bold italic uppercase tracking-tighter">Finalizing your receipt...</CardTitle>
            <CardDescription className="text-lg">
              Nova is synchronizing your settlement data with the credit bureau.
            </CardDescription>
          </CardHeader>
          <CardContent className="py-8 text-center">
            <p className="text-slate-500 font-medium">Please wait while we secure your documents. This usually takes a few seconds.</p>
          </CardContent>
          <CardFooter className="flex flex-col gap-3">
            <Button asChild className="w-full h-12 text-lg font-bold bg-indigo-600 hover:bg-indigo-700 shadow-lg shadow-indigo-100">
              <Link href={`/resolve/${caseId}`}>
                Refresh Status
              </Link>
            </Button>
          </CardFooter>
